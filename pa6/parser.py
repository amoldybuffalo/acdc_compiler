...
def parse_expression(ts: TokenStream) -> ASTNode:
    ...
    while ts.peek().tokentype != TokenType.EOF:
        ...
        if tok.tokentype == TokenType.INTLIT:
            ...
            valstack.append(IntLitNode(tok.value))
            continue
        ...
        if tok.tokentype == TokenType.FLOATLIT:
            ...
            valstack.append(FloatLitNode(tok.value))
            continue
        ...
        if tok.tokentype in operatortypes:
            ...
            while len(opstack) > 0 and opstack[-1].tokentype in operatortypes:
                ...
                if top_prec > inc_prec or (top_prec == inc_prec and leftassoc[top.tokentype]): #remember to do associativity
                  reduce(opstack, valstack)
                else:
                   break
...
def reduce(opstack: list, valstack: list) -> None:
    ...
    if opstack[-1] != None: 
        ...
        if len(valstack) >= 2:
            rhs = valstack.pop()
            lhs = valstack.pop()
            if isinstance(lhs, FloatLitNode) or isinstance(rhs, FloatLitNode) or (isinstance(lhs, BinOpNode ) and lhs.result_type == float) or (isinstance(lhs, BinOpNode ) and rhs.result_type == float):
                result_type = float
            else:
                result_type = int
            b = BinOpNode(op.tokentype, lhs, rhs, result_type)
            valstack.append(b)
