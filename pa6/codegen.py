...
def stmtcodegen(statement: ASTNode) -> InstructionList:
    ...
    if isinstance(statement, IntDclNode) or isinstance(statement, FloatDclNode):
        return code

    if isinstance(statement, IntLitNode) or isinstance(statement, FloatLitNode):
        code.append(str(statement.value).replace('-', '_'))
        return code
